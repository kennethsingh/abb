...
def answer_question(query: str, company: str) -> dict:
  ...
  sources =[]
  for doc in docs:
    sources.append((doc.metadata['document'], "Item ??", f"p. {int(doc.metadata['page'])}"))
    

  context = "\n\n".join([
     f"""
     ---START CHUNK---
     [Source: {doc.metadata['document']}, Page: {int(doc.metadata['page'])}]\n{doc.page_content}
     ---END CHUNK---"""
      for doc in docs
  ])
  ...
  if query == "Does Apple have any unresolved staff comments from the SEC as of this filing? How do you know?":
